...
function empower(lan,wan,admin)
    -- @PM chenyong, avoid collisions with 'parent control'
    --[[
...
function getremotemac()
    local XQFunction = require("xiaoqiang.common.XQFunction")
    local remote_addr = luci.http.getenv("HTTP_X_FORWARDED_FOR")
    if XQFunction.isStrNil(remote_addr) then
        remote_addr = luci.http.getenv("REMOTE_ADDR") or ""
    end
...
function _webAccessAllowed(auth)
    if not auth then
        return true
    end
    if auth == "htmlauth" then
        local mac = getremotemac()
        local uci = require("luci.model.uci").cursor()
        local admin = uci:get("macfilter","mode","admin") or ""
        if admin == "whitelist" then
            uci:foreach("macfilter", "mac",
                function(s)
                    if s.mac and string.lower(s.mac) == string.lower(mac) and s.admin == "no" then
                        return false
                    end
                end
            )
...
function dispatch(request)
    ...
    -- 系统语言ACCEPT_LANGUAGE和语言包匹配
    local httpLang = http.getenv("HTTP_ACCEPT_LANGUAGE") or ""
    local confLangs = {zh_cn = true, zh_tw = true, zh_hk = true}
    local hasLang = false
    for lpat in httpLang:gmatch("[%w-]+") do
        lpat = lpat and lpat:gsub("-", "_")
        lpat = string.lower(lpat)
        if confLangs[lpat] then
            hasLang = true
            break
        end
    end
    ...
    if not _webAccessAllowed(track.sysauth_authenticator) then
        errorpage(403, "Access barred!")
        return
    end
