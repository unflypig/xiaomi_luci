...
-- @see close
function write(content, src_err, jsonp, content_length)
    if not content then
        if src_err then
            error(src_err)
        else
    ...
    elseif #content == 0 then
        ...
        if not context.eoh then
            ...
            if content_length and not context.headers["Content-Length"] then
...
function write_jsonp(x, callback)
    if x == nil then
        write("null")
    elseif type(x) == "table" then
        local json = require("luci.json")
        if callback then
            write(callback.."("..json.encode(x)..");", nil, true)
        else
            write("jsonpHandler("..json.encode(x)..");", nil, true)
        end
    elseif type(x) == "number" or type(x) == "boolean" then
        if (x ~= x) then
            -- NaN is the only value that doesn't equal to itself.
        ...
        if callback then
            write(callback..'("%s");' % tostring(x):gsub('["%z\1-\31]', function(c)
                return '\\u%04x' % c:byte(1)
            end), nil, true)
        else
            write('jsonpHandler("%s");' % tostring(x):gsub('["%z\1-\31]', function(c)
                return '\\u%04x' % c:byte(1)
            end), nil, true)
        end
    end
