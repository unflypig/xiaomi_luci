...
function setAllWifi()
    ...
    local succeed
    if on1 == 0 then
        succeed = XQWifiUtil.setWifiBasicInfo(1, nil, nil, nil, nil, nil, nil, on1, nil, bsd)
    else
        if ssid1 and on1 then
            code1 = XQWifiUtil.checkSSID(ssid1,31)
            if code1 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(1, ssid1, password1, encryption1, channel1, txpwr1, hidden1, on1, bandwidth1, bsd)
            else
                code = code1
            end
        end
    ...
    if on2 == 0 then
        succeed = XQWifiUtil.setWifiBasicInfo(2, nil, nil, nil, nil, nil, nil, on2, nil, bsd)
    else
        if ssid2 and on2 then
            code2 = XQWifiUtil.checkSSID(ssid2,31)
            if code2 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(2, ssid2, password2, encryption2, channel2, txpwr2, hidden2, on2, bandwidth2, bsd)
            else
                code = code2
            end
        end
