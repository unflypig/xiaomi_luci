        ...
        for _,ips in pairs(v['ip_list'] or {}) do
            ...
            if valid_ip then
                ...
                if (max_up < 8 or max_up > math.ceil(cfg.bands.UP)) and
                    ...
                    -- 如果未指定单方限速，则设置为最大限速值
                    if max_up < 8 then max_up = cfg.bands.UP end
                    if max_down < 8 then max_down = cfg.bands.DOWN end
                    -- 更新device状态信息
...
local function apply_rule_for_single_device(tblist, devs, act, parent, device)
    ...
    for dir,v in pairs(devs) do
        ...
        local rate,ceil=rate_for_each[dir], math.ceil(limit[dir])
        local buffer=math.ceil(ceil*g_htb_buffer_data)
        ...
        elseif act == 'change' then
            expr = string.format(
                "%s %s dev %s parent %s:%s classid %s:%s htb rate %s%s ceil %s%s burst %d cburst %d quantum %s",
                const_tc_class, act, dev, flow_id, dec2hexstr(parent), flow_id, dec2hexstr(host_root_id), rate, UNIT,
                ceil, UNIT, buffer, buffer, quantum)
        ...
        for _type,cls in pairs(CLASS_HIER_host_ext) do
            ...
            if act == 'del' then
                ...
                local buffer=math.ceil(lceil*g_htb_buffer_data)
                local lprio=cfg.flow.inner[cls['type']]
                ...
                expr = string.format("%s %s dev %s parent %s:%s classid %s:%s htb rate %s%s ceil %s%s prio %s " ..
                    ...
                    flow_id, dec2hexstr(class_flow_id), lrate, UNIT, lceil, UNIT, lprio , quantum, buffer, buffer)
                if act == 'change' then
                    table.insert(tblist, 1, expr)
                else
                    table.insert(tblist, expr)
...
local function apply_service_main_qdisc_class_filter(devs, bands)
    ...
    for dir, v in pairs(devs) do
        ...
        local buffer = math.ceil(tonumber(ratelimit)*g_htb_buffer_data)
        ...
        expr = string.format("%s %s dev %s parent %s: classid %s:%s htb rate %s%s quantum %d burst %d cburst %d",
            const_tc_class, act, dev, flow_id, flow_id, root_class_id, ratelimit, UNIT, quan_v, buffer, buffer)
        table.insert(tblist,expr)
        ...
        for _cls_name, _chd in pairs(CLASS_HIER['child']) do
            ...
            expr=string.format('%s %s dev %s parent %s:%s classid %s:%s htb rate %s%s ceil %s%s ' ..
                ...
                dec2hexstr(class_id), lrate, UNIT, lceil, UNIT, lprio, quan_v, buffer, buffer)
            table.insert(tblist, expr)
            ...
            elseif _cls_name == 'host' then  -- 主网络
                ...
                for _service_name, _service in pairs(CLASS_HIER_host_ext) do
                    ...
                    local _this_prio=cfg.flow.inner[_service_name]  -- TODO
                    expr = string.format("%s %s dev %s parent %s:%s "..
                        ...
                        flow_id, dec2hexstr(_this_class), _this_rate, UNIT, _this_ceil, UNIT, _this_prio, quan_v, buffer, buffer)
                    table.insert(tblist, expr)
...
local function change_single_node_limit(node_name, devs, bands)
    local tblist={}
    ...
    for _dir,v in pairs(devs) do
        ...
        -- 如果ceil为0,则取limit
        local ceil_limit=class.ceil
        if class.ceil ==0 then
            ceil_limit = class.limit[_dir]
        end

        if ceil_limit == 0 then
            lceil = ratelimit
        elseif ceil_limit <= 1 then
            lceil = math.ceil(ratelimit * ceil_limit)
        else
            lceil = ceil_limit
        end
...
-- 检查是否需要更新访客网络限速
local function check_if_need_update_guest_network(devs, bands)
    ...
    if flag then
        change_single_node_limit('guest', devs, bands)
    end
end

local function check_if_need_update_xq_network(devs, bands)
    local strlog,flag='', false
    ...
    if flag then
        change_single_node_limit('xq', devs, bands)
    end
end
...
function qdisc_df.read_qos_config()
    ...
    -- 读取guest的配置
    if not read_qos_guest_config() then
        logger(3,'read_qos_guest_config failed.')
        return false
    end
...
function qdisc_df.apply(origin_qdisc, bands,devs, clean_flag)
    ...
    if not _clean_flag and origin_qdisc and origin_qdisc == THIS_QDISC then -- qdisc未变化，则不清除
        _clean_flag = false
    else
        _clean_flag = true
    end
...
function qdisc_df.update_counters(devs)
    ...
    for _ip,_dev in pairs(device_list) do
        data[_ip]={
            MAC=_dev.mac,
            UP={min=0,min_per=0,min_cfg=0,max=_dev.limit.UP .. 'Kbit',max_per=0,max_cfg=0},
            DOWN={min=0,min_per=0,min_cfg=0,max=_dev.limit.DOWN .. 'Kbit',max_per=0,max_cfg=0},
        }
    end
