...
local function _mi_wifi_info_restore(info)
    ...
    if info then
        ...
        if w1 then
            wifi.setWifiBasicInfo(1, w1.ssid, w1.password, w1.encryption, w1.channel, w1.txpwr, w1.hidden, on1, w1.bandwidth, w1.bsd)
        end
        if w2 then
            wifi.setWifiBasicInfo(2, w2.ssid, w2.password, w2.encryption, w2.channel, w2.txpwr, w2.hidden, on2, w2.bandwidth, w2.bsd)
        end
    end
end
...
local function _mi_lan_info_restore(info)
    ...
    if info then
        ...
        if dhcp then
            uci:section("dhcp", "interface", "lan", dhcp)
            uci:commit("dhcp")
        end
    end
end
...
local function _mi_arn_info_restore(info)
    ...
    if info then
        ...
        local mlist = info["list"]
        if mode ~= 0 then
            wifi.setWiFiMacfilterModel(true, mode - 1)
            if mlist then
                wifi.editWiFiMacfilterList(mode - 1, mlist, 0)
            end
        end
    end
end
