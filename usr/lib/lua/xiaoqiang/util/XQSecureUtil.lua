...
function decCiphertext(user, ciphertext)
    if XQFunction.isStrNil(ciphertext) or ciphertext:match("\"") or ciphertext:match(" ") then
        return nil
    end
...
--     if XQFunction.isStrNil(pwd) then
--         return 1
--     end
--     local modes = 0
...
--     end
--     local strong = _bitTotal(modes)
--     if pwd and string.len(pwd) < 8 then
--         if strong == 3 then
--             return 2
--         else
--             return strong
...
function checkStrong(pwd)
    ...
    if #pwd < 6 then
        ...
        local p = weight.l * math.sqrt((#pwd - 6)/2)
        if p > 2 then
            p = 1
        end
        score = score + p
