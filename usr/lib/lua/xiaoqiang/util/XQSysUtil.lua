...
function backupSysLog()
    ...
    if not sane() then
        ...
        os.execute("rm "..backuppath.."*.zip")
    end
    os.execute("/usr/sbin/log_collection.sh >/dev/null 2>/dev/null")
    if fs.access("/tmp/log.zip") then
        local filename = os.date("%Y-%m-%d--%X",os.time())..".zip"
        os.execute("cp /tmp/log.zip "..backuppath..filename)
        os.execute("rm /tmp/log.zip")
...
function setSysTime(time, tzone)
