...
function uploadLogFile(logfile, logtype, logkey)
    ...
    local key = Mime.b64(logkey) or Mime.b64(generateLogKey())
    local filebase64str = luci.util.exec("/usr/bin/base64 "..logfile)
    local blob = {["column"]=column, ["$"]=string.gsub(filebase64str,"\n","")}
    local cell = {blob}
...
local LOG_URL = "https://log.miwifi.com/log/lite/common/%s"
local LOG_CMD = "curl -k -i -X POST -F 'id=%s' -F '_n=%s' -F '_t=%s' -F 'extra_data={\"version\":\"%s\", \"sn\":\"%s\", \"suffix\":\"zip\"}' -F 'mode=%s' -F 'key=%s' -F 'payload=@/tmp/log.zip' %s"
...
function uploadLogV2(key)
    local LuciSys = require("luci.sys")
    local id = getDeviceId() or ""
    local _n = LuciSys.uniqueid(10)
    local _t = os.time()
    local version = XQSysUtil.getRomVersion()
    local sn = getSN() or ""
    local hardware = XQSysUtil.getHardware() or ""
    local url = string.format(LOG_URL, hardware)
    local mode = ""
    if key then
        mode = "useRomTime"
        _t = key:match("%-(%d+)$")
    ...
    local cmd = string.format(LOG_CMD,
        id, _n, _t, version, sn, mode, key, url
    )
...
local XIAOQIANG_ECHOS = "/rs/grayupgrade/v2/r01"
...
function checkEcosUpgrade(version, channel, sn, countryCode)
    local LuciJson = require("json")
    local params = {
        {"version", version},
        {"hardware", "r01"},
    ...
    local subUrl = XIAOQIANG_ECHOS.."?"..queryString
    ...
    if tonumber(resultjson["code"]) == 0 then
        local result = {}
        if resultjson.data and resultjson.data.upgradeInfo and resultjson.data.upgradeInfo.link then
            result["needUpdate"] = 1
            result["downloadUrl"] = resultjson.data.upgradeInfo.link
            result["fullHash"] = resultjson.data.upgradeInfo.hash
            result["fileSize"] = resultjson.data.upgradeInfo.size
            result["version"] = resultjson.data.upgradeInfo.toVersion
            result["description"] = resultjson.data.upgradeInfo.description
        else
            result["needUpdate"] = 0
        end
        return result
